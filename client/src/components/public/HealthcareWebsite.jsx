...
const navItems = [
  ...
  ['Doctors / Team', '#team'], ['Facilities', '#facilities'], ['Gallery', '#gallery'], ['Reviews', '#reviews'], ['Contact', '#contact'],
];
...
export default function HealthcareWebsite({ place, mapsUrl, onShare, onReport, onDelete }) {
  ...
  const actionLinks = [
    ...
    place.address && ['Directions', mapsUrl || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(place.address)}`, 'bg-slate-800'],
  ...
  ].filter((link) => Array.isArray(link) && typeof link[1] === 'string' && link[1].trim());

  const currentNavItems = videos.length > 0
    ? [...navItems.slice(0, 8), ['Videos', '#videos'], ...navItems.slice(8)]
    : navItems;
  ...
  return <div className={isFitness ? 'min-h-screen bg-[#f2fbf7] text-slate-900' : 'min-h-screen bg-white text-slate-900'}>
    <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/95 shadow-sm backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
        ...
        <div className="hidden items-center gap-2 md:flex">
          <a href="/" className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-700 transition hover:border-sky-300 hover:text-sky-700">← Back to G-Pages</a>
          {appointmentUrl && <a href={appointmentUrl} target="_blank" rel="noreferrer" className="rounded-lg bg-teal-600 px-3 py-2 text-xs font-bold text-white">Appointment</a>}
        </div>
        <div className="flex items-center gap-2 md:hidden">
          <a href="/" className="rounded-lg border border-slate-200 bg-white px-2.5 py-2 text-[10px] font-bold text-slate-700">← G-Pages</a>
          <button type="button" onClick={() => setMenuOpen((open) => !open)} className="rounded-lg border border-slate-200 px-3 py-2 text-sm" aria-label="Open navigation">☰</button>
        </div>
      </div>
    ...
    <main>
      ...
      <section id="reviews" className="mx-auto max-w-7xl px-4 pb-12 sm:px-6 lg:px-8"><ReviewsSection placeId={place._id} /></section>
    </main>
