...
const typeFor = { Restaurants: 'restaurant', 'Coffee Shops': 'coffee-shop', 'Sweet Shops & Bakery': 'bakery', 'Catering Services': 'catering', 'Food Processing': 'food-processing' };
...
export default function FoodBusinessCreateForm({ categories, onBack }) {
  ...
  return <div><div className="container-page mx-auto max-w-5xl px-5 pt-8 sm:px-8"><label className="block max-w-md text-sm font-semibold text-ink/70">Food & Dining subtype<select value={subcategoryId} onChange={(event) => setSubcategoryId(event.target.value)} className="mt-1 w-full rounded border border-line bg-white px-3 py-2.5">{subcategories.map((item) => <option key={item._id} value={item._id}>{item.name}</option>)}</select></label></div><FoodBusinessEditor place={draft} businessType={typeFor[selected.name]} create onBack={onBack} /></div>;
}
