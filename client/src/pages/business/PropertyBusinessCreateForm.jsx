...
export default function PropertyBusinessCreateForm({ categories, onBack }) {
  ...
  return <div><div className="mx-auto max-w-5xl px-5 pt-8 sm:px-8"><label className="block max-w-md text-sm font-semibold text-[#31465a]">Business type<select value={selectedId} onChange={(event) => setSelectedId(event.target.value)} className="mt-1 w-full rounded-xl border border-[#d9e0e6] bg-white px-3 py-2.5">{options.map((item) => <option key={item._id} value={item._id}>{item.name}</option>)}</select></label></div><PropertyBusinessForm key={types[selected.name]} categoryId={selected._id} propertyType={types[selected.name]} create onBack={onBack} /></div>;
}
